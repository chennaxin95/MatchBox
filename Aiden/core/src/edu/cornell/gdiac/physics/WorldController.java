 ...
 * really a mini-GameEngine in its own right. The only thing that it does not do
 * is create a GameCanvas; that is shared with the main application.
 *
...
public abstract class WorldController implements Screen {
	 ...
	 *
	 * @param the
	 *            canvas associated with this controller
	 */
	 ...
	 *
	 * The canvas is shared across all controllers. Setting this value will
	 * compute the drawing scale from the canvas size.
	 *
	 * @param value
	 ...
	 *
	 * The game world is scaled so that the screen coordinates do not agree with
	 * the Box2d coordinates. The bounds are in terms of the Box2d world, not
	 * the screen.
	 */
	 ...
	 *
	 * @param width
	 *            The width in Box2d coordinates
	 * @param height
	 *            The height in Box2d coordinates
	 * @param gravity
	 ...
	 *
	 * @param bounds
	 *            The game bounds in Box2d coordinates
	 * @param gravity
	 *            The gravitational force on this Box2d world
	 */
	...
	public void draw(float delta) {
		...
		if (complete && !failed) {
			...
			canvas.drawTextCentered("VICTORY!", af.displayFont, 0.0f);
			canvas.end();
		} else if (failed) {
			af.displayFont.setColor(Color.RED);
			canvas.begin(); // DO NOT SCALE
			canvas.drawTextCentered("FAILURE!", af.displayFont, 0.0f);
			canvas.end();
		}
