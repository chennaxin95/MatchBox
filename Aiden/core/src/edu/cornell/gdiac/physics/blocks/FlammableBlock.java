...
public class FlammableBlock extends BlockAbstract
	...
	public FlammableBlock(float width, float height, float spreadRate,
			float burnRate) {
		super(width, height);
		flammable = true;
		burning = false;
	...
	public FlammableBlock(float x, float y, float width, float height,
			float spreadRate,
			float burnRate) {
	...
	public void updateBurningState(float dt) {
		// TODO Auto-generated method stub
		if (this.isBurning()) {
			burnTimer -= dt * burnBonus;
			spreadTimer -= dt;
			// burnBonus should only be higher if Aiden is touching
			burnBonus = 1f;
		}
		checkBurnt();
	}
	...
	public void setBurnBonus(float value) {
		this.burnBonus = value;
	}
		...
		if (texture != null) {
			if (isBurnt()) {
				canvas.draw(texture, Color.BLACK, origin.x, origin.y,
						getX() * drawScale.x, getY() * drawScale.x, getAngle(),
						1, 1);
			} else if (isBurning()) {
				Color c = new Color();
				if (getBurnRatio() > 0.3) {
